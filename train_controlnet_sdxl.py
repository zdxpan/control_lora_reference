...
def prepare_train_dataset(dataset, accelerator):
    ...
    def preprocess_train(examples):
        # examples = {}  dict no attri of map will case a error
        # img = Image.open(BytesIO(image_data))
        images = [Image.open(BytesIO(image["bytes"])) for image in examples["image"]]
        images = [image.convert("RGB") for image in images]
        images = [image_transforms(image) for image in images]

        examples.pop("image")
        examples["pixel_values"] = images
        examples["conditioning_pixel_values"] = images
        examples["text"] = examples["text"]
        return examples

    def preprocess_train2(examples):
...
def main(args):
    ...
    if args.use_8bit_adam:
        ...
        optimizer_class = lambda params, lr, **b: torch.optim.AdamCM(params, lr=lr)
    else:
    ...
    train_dataset = load_dataset("/home/dell/workspace/dataset/laion_data/", data_files="train-00019-of-00080-c30db6a160deb724.parquet")
    ...
    for s, it in enumerate(train_dataloader):
        break
    print(">> if it contain embeds------------")
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        cnet = accelerator.unwrap_model(controlnet)
...
if __name__ == "__main__":
    ...
    args = Namespace(
        ...
        controlnet_model_name_or_path='/home/dell/workspace/controlnet/control_train/f1checkpoint-400',
        ...
        revision=None,
        dtype=torch.float16,
        # dtype=torch.float32,   # seemed good then half precesion
        # -- data--------------,
        train_batch_size=2,
        ...
        report_to='wandb',
        proportion_empty_prompts=0.5,  # 一半的图不给提示
        push_to_hub=False,
        hub_token=None,
        ...
        validation_image = list(b["image"]),
        validation_prompt = list(b["prompt"]),
        alidation_steps=1,
